...
describe('DiffService', () => {
    ...
    const baseHtml1 =
        ...
        brMarkup(11) +
        'Line 11</p>';
    const baseHtmlDom1 = DiffService.htmlToFragment(baseHtml1);
...
<p>' +
    ...
    const baseHtmlDom2 = DiffService.htmlToFragment(baseHtml2);
    ...
    DiffService.insertInternalLineMarkers(baseHtmlDom1);
    DiffService.insertInternalLineMarkers(baseHtmlDom2);
    ...
    describe('extraction of lines', () => {
        it('locates line number nodes', () => {
            let lineNumberNode = DiffService.getLineNumberNode(baseHtmlDom1, 4);
        ...
        it('finds the common ancestor', () => {
            ...
            commonAncestor = DiffService.getCommonAncestor(fromLineNode, toLineNode);
        ...
        it('renders DOMs correctly (1)', inject([DiffService], (service: DiffService) => {
            const lineNo = DiffService.getLineNumberNode(baseHtmlDom1, 7),
        ...
        it('renders DOMs correctly (2)', inject([DiffService], (service: DiffService) => {
            const lineNo = DiffService.getLineNumberNode(baseHtmlDom1, 9),
                greatParent = lineNo.parentNode.parentNode,
                lineTrace = [lineNo.parentNode, lineNo];
...
<LI>Punkt 4.3</LI>\
                ...
                const outHtml = LinenumberingService.nodesToHtml([<Element>out[0]]);
                expect(outHtml).toBe(
    ...
    describe('detecting the type of change', () => {
        it('detects a simple insertion', () => {
            ...
            const calculatedType = DiffService.detectReplacementType(htmlBefore, htmlAfter);
            expect(calculatedType).toBe(ModificationType.TYPE_INSERTION);
        });

        it('detects a simple insertion, ignoring case of tags', () => {
            const htmlBefore = '<p>Test 1</p>',
                htmlAfter = '<P>Test 1 Test 2</P>' + '\n' + '<P>Test 3</P>';
            const calculatedType = DiffService.detectReplacementType(htmlBefore, htmlAfter);
            expect(calculatedType).toBe(ModificationType.TYPE_INSERTION);
        ...
        it('detects a simple insertion, ignoring trailing whitespaces', () => {
        ...
        it('detects a simple insertion, ignoring spaces between UL and LI', () => {
        ...
        it('detects a simple insertion, despite &nbsp; tags', () => {
        ...
        it('detects a simple deletion, ignoring case of tags', () => {
            const htmlBefore = '<p>Test 1 Test 2</p>' + '\n' + '<p>Test 3</p>',
                htmlAfter = '<P>Test 1</P>';
            const calculatedType = DiffService.detectReplacementType(htmlBefore, htmlAfter);
            expect(calculatedType).toBe(ModificationType.TYPE_DELETION);
        ...
        it('detects a simple deletion, ignoring trailing whitespaces', () => {
        ...
        it('detects a simple replacement', () => {
            const htmlBefore = '<p>Test 1 Test 2</p>' + '\n' + '<p>Test 3</p>',
                htmlAfter = '<p>Test 1</p>' + '\n' + '<p>Test 2</p>' + '\n' + '<p>Test 3</p>';
            const calculatedType = DiffService.detectReplacementType(htmlBefore, htmlAfter);
    ...
    describe('diff normalization', () => {
        it('uppercases normal HTML tags', () => {
            const unnormalized = 'The <strong>brown</strong> fox',
                normalized = DiffService.normalizeHtmlForDiff(unnormalized);
