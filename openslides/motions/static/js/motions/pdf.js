...
.factory('MotionContentProvider', [
    ...
    function($q, operator, gettextCatalog, PDFLayout, PdfMakeConverter, ImageConverter,
        ...
        var createInstance = function(motion, motionVersion, params) {
            ...
            var titleChange = motion.changeRecommendations.find(function(change) { return change.isTitleRecommendation(); });
            var title, titlePlain;
            if (titleChange && (params.changeRecommendationMode === "changed" || params.changeRecommendationMode === 'agreed' && !titleChange.rejected)) {
                titlePlain = titleChange.text;
            } else {
                titlePlain = motion.getTitle(motionVersion);
            }
            title = PDFLayout.createTitle(gettextCatalog.getString('Motion') + identifier + ': ' + titlePlain);
